...
class TutorGroupClassController {

    private $TutorGroupClassModel;

    public function __construct() {
        $this->TutorGroupClassModel = new TutorGroupClass();
    }
    ...
    public function create() {
        ...
        try {
            if ($this->TutorGroupClassModel->addClass($_POST['title'], $_POST['description'], $_POST['first_day'], $_POST['first_time'], $_POST['second_day'], $_POST['second_time'], $_POST['number_of_classes'], $_POST['duration'], $_POST['tutor'], $_POST['pricing'], $cover_image_path)) {
                ...
                header('Location: ../../src/views/tutor/group-classes.php');
                exit; 
            } else {
    ...
    public function handlePaymentSuccess($sessionId) {
        try {
            ...
            $TutorGroupClassModel = new TutorGroupClass();

            // Use 'paid' as the status for successful payment
            $status = 'paid';

            // Process the enrollment
            $result = $TutorGroupClassModel->enrollStudent($studentId, $classId, date('Y-m-d H:i:s'), $amountPaid / 100, $status);
    ...
    public function TutorGetAllGroupClasses() {
        try {
            return $this->TutorGroupClassModel->TutorGetAllGroupClasses();
        } catch (Exception $e) {
            echo $e->getMessage();
        }
    }
    ...
    public function edit($class_id) {
        try {
            $class = $this->TutorGroupClassModel->getGroupClassById($class_id);
            ...
            include '../../src/views/tutor/edit-group-class.php';
    ...
    public function update() {
        ...
        try {
            if ($this->TutorGroupClassModel->updateGroupClass($_POST['class_id'], $_POST['title'], $_POST['description'], $_POST['first_day'], $_POST['first_time'], $_POST['second_day'], $_POST['second_time'], $_POST['number_of_classes'], $_POST['duration'], $_POST['tutor'], $_POST['pricing'], $cover_image_path)) {
    ...
    public function draft($class_id) {
        try {
            return $this->TutorGroupClassModel->draftClass($class_id);
        } catch (Exception $e) {
            echo $e->getMessage();
            return false;
        }
    ...
    public function delete($class_id) {
        try {
            return $this->TutorGroupClassModel->deleteClass($class_id);
        } catch (Exception $e) {
            echo $e->getMessage();
            return false;
        }
    ...
    public function enrollStudent($data) {
        try {
            $enrollmentResult = $this->TutorGroupClassModel->enrollStudent($data['student_id'], $data['class_id'], $data['date_enrolled'], $data['amount_paid'], $data['status']);
            if ($enrollmentResult) {
                return true;
            } else {
    ...
    public function enrollStudentManual($data) {
        try {
            ...
            $enrollmentResult = $this->TutorGroupClassModel->enrollStudent(
                $data['student_id'],
                $data['class_id'],
                $data['date_enrolled'],
                $data['amount_paid'],
    ...
    public function getEnrolledStudentsByClassId($class_id) {
        try {
            $students = $this->TutorGroupClassModel->getEnrolledStudentsByClassId($class_id);
            header('Content-Type: application/json');
    ...
    public function saveZoomMeeting($class_id, $zoom_link, $scheduled_date, $scheduled_time) {
        try {
            ...
            $saveResult = $this->TutorGroupClassModel->addZoomMeeting($class_id, $zoom_link, $scheduled_date, $scheduled_time);
    ...
    public function getZoomMeetingsByClassId($class_id) {
        try {
            $meetings = $this->TutorGroupClassModel->getZoomMeetingsByClassId($class_id);
            return $meetings;
        } catch (Exception $e) {
            header('HTTP/1.1 500 Internal Server Error');
            echo json_encode(['status' => 'error', 'message' => $e->getMessage()]);
    ...
    public function getAllStudents() {
        return $this->TutorGroupClassModel->getAllStudents();
    }
}

$controller = new TutorGroupClassController();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif (isset($_GET['action']) && $_GET['action'] == 'list') {
        $allClasses = $controller->TutorGetAllGroupClasses();
        // At this point, you can include the view which lists the group classes
        include '../../src/views/tutor/group-classes.php';
    }
