                    ...
                    <form method="GET" action="">
                        <div class="searchTop">
                            <div class="searchTop-left">
                                <!-- Removed the role filter as it's specific to tutors -->

                                <p>All Tutors: <?= $totalTutorsCount ?></p>
                                
                                <!-- If you still want to search by tutor subscriptions, you can uncomment this:
                                <select class="inp ms-3" name="tutors_subscription_filter" id="tutors_subscription_filter">
                                    ...
                                    <option value="Master Class Package">Master Class Package</option>
                                </select>
                                -->
                            </div>
                            <div class="searchTop-right ms-3">
                                <div>
                                    <!-- Updated the name of the search input to "tutors_search_term" to make it distinct -->
                                    <input class="inp" type="text" name="tutors_search_term" placeholder="Name or Username">
                                </div>
                    ...
                    <div class="table-responsive">
                        <table class="table theme-table">
                            <thead>
                                ...
                                <th>EMAIL <span><img src="./assets/images/filter.png" alt=""></span></th>
                                <th>LAST SEEN <span><img src="./assets/images/filter.png" alt=""></span></th>
                                <th>ACTION</th>
                            </thead>
                            <tbody>
                                <?php foreach ($allTutors as $Tutors): ?>
                                <tr>
                                    <td><strong><?= $Tutors['first_name'] . ' ' . $Tutors['last_name'] ?></strong> <br> <?= $Tutors['email'] ?></td>
                                    <td><?= $Tutors['email'] ?></td>
                                    <td><?= $Tutors['last_login_date'] ? $Tutors['last_login_date'] : 'Not available' ?></td>
                                    <td><a href="manage-tutors-details.php?email=<?= $Tutors['email'] ?>&username=<?= $Tutors['username'] ?>">Manage</a></td>
                                </tr>
                                <?php endforeach; ?>
                            </tbody>
                            <tfoot>
                                <tr>
                                    <td colspan="6">
                                        <span>Showing <?= (($tutorsPage - 1) * $tutorsLimit) + 1 ?>-<?= min($tutorsPage * $tutorsLimit, $totalTutorsCount) ?> of Total <?= $totalTutorsCount ?></span>
                                        <span class="table-nav">
                                            <a href="?tutors_page=<?= max(1, $tutorsPage - 1) ?>"><i class="fa-solid fa-arrow-left"></i></a>
                                            <span><?= $tutorsPage ?></span>
                                            <a href="?tutors_page=<?= min(ceil($totalTutorsCount / $tutorsLimit), $tutorsPage + 1) ?>"><i class="fa-solid fa-arrow-right"></i></a>
                                        </span>
                                    </td>
                                </tr>
                            </tfoot>
